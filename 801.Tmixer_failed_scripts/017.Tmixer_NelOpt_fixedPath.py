...
not_to_opt      = '(    (x[1] <= '+str(mesh_d*0.0)+'*sin(x[0]) +'+str(mesh_DD/2.0 +mesh_d*0.2/2.0)+') '\
                + '  && (x[1] >= '+str(mesh_d*0.0)+'*sin(x[0]) +'+str(mesh_DD/2.0 -mesh_d*0.2/2.0)+')    )'
dx_not_to_opt           = 1
...
AMP_g = Constant(1)
AMP_u = Constant(1)
J  = (a*GAM -a_obj)*(a*GAM -a_obj)*ds(ds_outlet) \
   - inner( outer(grad(GAM),u)/(GAM), outer(grad(GAM),u)/(GAM)  )   *dx
