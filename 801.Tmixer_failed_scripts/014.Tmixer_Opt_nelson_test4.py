...
not_to_opt  = '( x[0]<='+str( (    noOpt_len)*mesh_L )+' && x[1]>='+str(0.4*mesh_d )+' && x[1]<='+str(0.6*mesh_d )+')' \
         + '|| ( x[0]>='+str( (1.0-noOpt_len)*mesh_L )+' && x[1]>='+str(0.4*mesh_d )+' && x[1]<='+str(0.6*mesh_d )+')' \
         + '|| ( x[0]>='+str(0.45*mesh_L)+' && x[0]<='+str(0.55*mesh_L)+' && x[1]>='+str(0.45*mesh_d )+' && x[1]<='+str(0.55*mesh_d )+')'
to_opt      = '( x[0]> '+str(noOpt_len*mesh_L )+')'
inlet_1 = '( x[0]=='+str(1.0*mesh_P0)+' && x[1]>='+str(0.4*mesh_d )+' && x[1]<='+str(0.5*mesh_d )+' )'
inlet_2 = '( x[0]=='+str(1.0*mesh_P0)+' && x[1]>='+str(0.5*mesh_d )+' && x[1]<='+str(0.6*mesh_d )+' )'
outlet  = '( x[0]=='+str(1.0*mesh_L )+' && x[1]>='+str(0.4*mesh_P0)+' && x[1]<='+str(0.6*mesh_d )+' )'
walls   = 'on_boundary'    \
        + ' && !'+inlet_1  \
        + ' && !'+inlet_2  \
        + ' && !'+outlet
...
class straight_channel(Expression):
   def eval(self, value, x):
      A = mesh_d*0.20
      y1 = 0.5*mesh_d +A
      y2 = 0.5*mesh_d -A
...
gam = project(straight_channel(degree=1), U_AA)
...
J  = AMP_a*(a*gam -a_obj)*(a*gam -a_obj)*ux/(mesh_d)        *ds(ds_outlet) \
   #+ ((gam_high -gam)*(gam -gam_low)/0.25)**AMP_g/(mesh_d*mesh_L)   *dx \
   #+ AMP_u*inner( outer(u,grad(gam))/(N3*gam), grad(u)  )   *dx
