...
midlet1 = '( x[0]=='+str(0.5*mesh_L )+' && x[1]> '+str(1.0*mesh_P0)+' && x[1]<'+str(1.0*mesh_d)+' )'
...
N12 = Constant(120*cons_mu/(mesh_d/10)**2)
...
a_obj = Constant(0.5)
J  = Constant(100)*(a -a_obj)*(a -a_obj)*ds(ds_outlet ) \
   + Constant(100)*(a -a_obj)*(a -a_obj)*ds(ds_midlet1)
m  = Control(gam)
