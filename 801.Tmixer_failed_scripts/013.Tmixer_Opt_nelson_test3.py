...
not_to_opt  = '( x[0]<='+str(noOpt_len*mesh_L )+')'
...
class MassConstraint(InequalityConstraint):
   def __init__(self, MinFract):
      self.MinFract  = float(MinFract)
      self.MaxValue  = assemble( project(Constant(1),U_AA)     *dx(dx_not_to_opt) )
   ...
   def function(self, m):
      ...
      return [integral -self.MinFract*self.MaxValue]
   def jacobian(self, m):
