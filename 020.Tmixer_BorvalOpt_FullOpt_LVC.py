...
mesh_d   = 0.010          # 10mm
mesh_DD  = 0.050          # largura para otimizacao
mesh_L   = 0.100          # 100mm
mesh_P0  = 0.0
...
subdomain_lenX    = mesh_L  /subdomain_qtdX
subdomain_lenY    = mesh_DD /subdomain_qtdY
for sub_idx in range(subdomain_qtdX):
   for sub_idy in range(subdomain_qtdY):
      pX_1     = subdomain_lenX *(sub_idx  ) -mesh_tol
      pX_2     = subdomain_lenX *(sub_idx+1) +mesh_tol
      pY_1     = subdomain_lenY *(sub_idy  ) -mesh_tol
      pY_2     = subdomain_lenY *(sub_idy+1) +mesh_tol
      sub_str  = '( x[0]>='+str(pX_1)+' ) && ( x[0]<='+str(pX_2)+' ) && '   \
               + '( x[1]>='+str(pY_1)+' ) && ( x[1]<='+str(pY_2)+' )    '
      CompiledSubDomain( sub_str ).mark (domain, sub_idy +subdomain_qtdY*sub_idx)
...
problem_constraints = [VolConstraint(MinFract, sub_idx) for sub_idx in range(subdomain_qtdX*subdomain_qtdY)]
