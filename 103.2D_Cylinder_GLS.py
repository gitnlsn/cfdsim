...
dsp_inlet,dsp_outlet,dsp_walls,dsp_obst,dsp_pRef = 0,1,2,3,4
...
F  = inner( RHO*dot(u,grad_u.T) +grad(p) -MU*div(grad_u),
...
assign(ans.sub(p_ux ), project(Constant(1E-2), FunctionSpace(mesh, FE) ) )
assign(ans.sub(p_uy ), project(Constant(1E-2), FunctionSpace(mesh, FE) ) )
