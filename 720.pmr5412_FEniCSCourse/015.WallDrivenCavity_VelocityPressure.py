...
FF = inner(dot(u,grad(u).T), v)     *dx \
   + inner(grad(u), grad(v))/RE     *dx \
   - div(v)*p                       *dx \
   + div(u)*q                       *dx

MM = inner( div(u), div(u) ) *dx \
   + inner( dot(u,grad(u).T) +grad(p) -div(grad(u)+ grad(u).T)/RE,
            dot(u,grad(u).T) +grad(p) -div(grad(u)+ grad(u).T)/RE  ) *dx

p_ux,p_uy,p_pp,p_ps,p_om = 0,1,2,3,4
left  = '( x[0]==0 )'
right = '( x[0]==1 )'
up    = '( x[1]==1 )'
down  = '( x[1]==0 )'
...
for re in range(100, 1001, 100):
   RE.assign(re)
   solve(FF==0, ans, bc)
   print ('L2 residual = {}'.format(assemble(MM)))
