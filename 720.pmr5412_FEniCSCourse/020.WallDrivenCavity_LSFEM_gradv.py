...
elem = MixedElement([FE_1, FE_1, FE_1, FE_1, FE_1, FE_1, FE_1, FE_1, FE_1])
U = FunctionSpace(mesh, elem)

ans = Function(U)
ux,uy,p,w,psi,uxx,uxy,uyx,uyy = split(ans)
x,y = 0,1
...
grd_u = as_tensor([[uxx, uxy], [uyx, uyy]])
u_dr  = as_vector([Constant(1), Constant(0)])
u_wl  = as_vector([Constant(0), Constant(0)])

RE = Constant(1)

Lct = inner(   div_u, div_u                           ) *dx
Lmt = inner(   dot(u,grd_u.T) +grad(p) -div(grd_u.T +grd_u)/RE,
               dot(u,grd_u.T) +grad(p) -div(grd_u.T +grd_u)/RE ) *dx \
    + inner(   grad(u) -grd_u, grad(u) -grd_u         ) *dx
...
p_ux,p_uy,p_pp,p_ww,p_si,p_uxx,p_uxy,p_uyx,p_uyy = 0,1,2,3,4,5,6,7,8
left  = '( x[0]==0 )'
right = '( x[0]==1 )'
up    = '( x[1]==1 )'
down  = '( x[1]==0 )'
...
J = derivative( FF, ans )
problem  = NonlinearVariationalProblem (FF, ans, bc, J)
solver   = NonlinearVariationalSolver  (problem)
parameters = solver.parameters.newton_solver
parameters['absolute_tolerance'  ] = 1E-12
parameters['relative_tolerance'  ] = 1E-13
parameters['maximum_iterations'  ] = 25

RE.assign(25)
solver.solve()

def plotxy_velocity(ans, foldername):
   ux,uy,p,ome,psi,uxx,uxy,uyx,uyy = ans.split()
...
def solve_re(cons_re, foldername):
   ...
   for re in range(int(last_re), cons_re+1, 10):
...
#plotxy_velocity         (ans, 'test')
#save_velocity_txt       (ans, 'test')
#save_streamVorticy_txt  (ans, 'test')

for re in range(30, 101, 10):
   RE.assign(re)
   solver.solve()

for cons_re in [1000,2500]:
   print('Solving for Re = {}'.format(cons_re))
   solve_re(cons_re, 'LSgrad_R'+str(res))
