...
class ChannelObstruction(Expression):
...
gam   = project(Constant(1.0), U_gam, name='permeability', annotate=True)
...
u_in  = Expression(('vin*x[1]*(1-x[1])/K','0'), vin=cons_vin, K=4*mesh_DD**2, degree=2)
BC1 = [
         DirichletBC(U.sub(p_uu), u_in, inlet_1),
         DirichletBC(U.sub(p_uu), u_in, inlet_2),
...
assign(ans.sub(p_pp), project(Constant(0.0      ), FunctionSpace(mesh, FE_P)) )
assign(ans.sub(p_aa), project(Constant(0.0      ), FunctionSpace(mesh, FE_P)) )
...
class LowerBound(Expression):
   ...
   def eval_cell(self, values, x, ufc_cell):
      ...
      #in_obstruction_zone = (x[0]>mesh_obX -mesh_obD) and (x[0]<mesh_obX +mesh_obD) and (x[1]>mesh_obY -mesh_obD) and (x[1]<mesh_obY +mesh_obD)
      #if in_obstruction_zone:
      #   values[0] = Constant(0.0)
