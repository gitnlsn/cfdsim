...
mesh_obX  = 0.2*mesh_L     # obstruction
mesh_obY  = 0.5*mesh_DD
mesh_obD  = 0.4*mesh_d
mesh_chY  = 0.5*mesh_DD    # channel not to opt
mesh_Cx   = mesh_obX       # initial circle
mesh_Cy   = mesh_obY
mesh_Rad  = 0.2*mesh_d
...
in_obstruction = '( (x[0]>'+str(mesh_obX -mesh_obD)+') && '\
               + '  (x[0]<'+str(mesh_obX +mesh_obD)+') && '\
               + '  (x[1]>'+str(mesh_obY -mesh_obD)+') && '\
               + '  (x[1]<'+str(mesh_obY +mesh_obD)+')      )'
in_channel     = '( (x[1]>'+str(mesh_chY -mesh_d  )+') && '\
               + '  (x[1]<'+str(mesh_chY +mesh_d  )+')      )'
not_to_opt     = in_channel +' && !'+in_obstruction
dx_not_to_opt  = 1
...
gam   = project(straigthChannel(degree=1), U_gam)
...
J  = \
   + AMP_a* (a2 -a_obj)*(a2 -a_obj)            *dx*dt[FINISH_TIME]
