...
vtk_dJ  = File(filename+'/sensibility.pvd')
...
J  = Functional( inner(a_nxt-0.5,a_nxt-0.5)*ds(ds_outlet)*dt )
print ('Functional: {}'.format(J))
dJ = compute_gradient(J, Control(alpha), forget=False)
vtk_dJ << dJ
plot(dJ, title='Gradient')
#interactive()
