...
cons_mu  = 8.5E-4        # 0.00085 N*s/m**2
cons_g   = 9.8
cons_vin = 5E-4
...
mesh_L   = 0.100          # 100mm
mesh_inP = 0.0; mesh_inP = mesh_DD*0.5 -(mesh_DD -mesh_d)*0.5 *mesh_inP
mesh_P0  = 0.0
mesh_tol = mesh_d *0.10

# ------ SIMULATION PARAMETERS ------ #
cons_re = cons_rho*cons_vin*mesh_d/cons_mu
cons_pe = mesh_d*cons_vin/cons_D
print('Velocity: {:.2e}'.format(cons_vin))
print('Reynolds: {:.2e}'.format(cons_re))
print('Peclet  : {:.2e}'.format(cons_pe))
...
inlet_1 = '( on_boundary && x[0]=='+str(0.00*mesh_L )                           \
                     + ' && x[1]>='+str(mesh_inP             -mesh_tol )        \
                     + ' && x[1]<='+str(mesh_inP +mesh_d*0.5 +mesh_tol )+' )'
...
# subdomain_len        = mesh_L /subdomain_qtd
# for sub_idx in range(subdomain_qtd):
#    tol      = subdomain_len *0.1
#    pos_1    = subdomain_len *(sub_idx  ) -tol
#    pos_2    = subdomain_len *(sub_idx+1) +tol
#    sub_str  = '( x[0]>='+str(pos_1)+') && ( x[0]<='+str(pos_2)+')'
#    CompiledSubDomain( sub_str ).mark (domain, sub_idx)

domain = CellFunction  ('size_t', mesh)
not_to_opt  =     '( 3*(x[0]-'+str(0.0*mesh_L  )+')*(x[0]-'+str(0.0*mesh_L  )+') '\
            + '      + (x[1]-'+str(1.0*mesh_inP)+')*(x[1]-'+str(1.0*mesh_inP)+') <= '+str((mesh_d/2.0 +mesh_tol)**2)+'    ) '\
            + ' || ( 3*(x[0]-'+str(1.0*mesh_L  )+')*(x[0]-'+str(1.0*mesh_L  )+') '\
            + '      + (x[1]-'+str(1.0*mesh_inP)+')*(x[1]-'+str(1.0*mesh_inP)+') <= '+str((mesh_d/2.0 +mesh_tol)**2)+'    )'
dx_not_to_opt           = subdomain_qtd
CompiledSubDomain( not_to_opt ).mark( domain, dx_not_to_opt )
...
class gam_wave(Expression):
   def eval(self, value, x):
      N = 4.0
      A = mesh_DD*0.5
      y1 = A*sin(2*pi*N*x[0] /mesh_L -pi/4.0*mesh_inP) +mesh_d/2.0 +mesh_tol +mesh_DD/2.0
      y2 = A*sin(2*pi*N*x[0] /mesh_L -pi/4.0*mesh_inP) -mesh_d/2.0 +mesh_tol +mesh_DD/2.0
...
class straigthChannel(Expression):
   def eval(self, value, x):
      y1 = mesh_DD*0.5 +mesh_d*0.5
      y2 = mesh_DD*0.5 -mesh_d*0.5
      is_permeable = x[1]>y2 and x[1]<y1
      if is_permeable:
         value[0] = 1.0
      else:
...
#gam = project(gam_wave(degree=1), U_AA, name='permeability')
gam = interpolate(gam_wave(degree=1), U_AA, name = "permeability", annotate = True)
#gam = project(straigthChannel(degree=1), U_AA)
#gam = project(Constant(1.0), U_AA)
...
he = CellSize(mesh)
Tsupg = (4/(cons_pe*he**2) +2*sqrt(inner(u,u))/he)**-1
...
def save_flow():
   ui = project(u,FunctionSpace(mesh,FE_U), annotate=False)
   pi = project(p,FunctionSpace(mesh,FE_P), annotate=False)
   ai = project(a,FunctionSpace(mesh,FE_P), annotate=False)
   ...
   vtk_pi << pi
   vtk_ai << ai
   uu = project(u*gam,FunctionSpace(mesh,FE_U), annotate=False)
   pp = project(p*gam,FunctionSpace(mesh,FE_P), annotate=False)
   aa = project(a*gam,FunctionSpace(mesh,FE_P), annotate=False)
...
vtk_dj = File(foldername+'/gradient.pvd')
fig = plot(gam, title='Gradient', mode='color')
...
AMP_a  = Constant(1.0E3)
AMP_u  = Constant(1.0E0)
AMP_g  = Constant(7.0E3)
J  = \
   + AMP_a* (a -a_obj)*(a -a_obj)*gam        *dx \
   + AMP_u* inner( u*alpha(gam),u )          *dx \
   + AMP_g* inner( grad(u), grad(u) )        *dx \
   #+ p*ds(ds_inlet1) + p*ds(ds_inlet2) - p*ds(ds_outlet) \
...
class VolConstraint(InequalityConstraint):
   def __init__(self, MinFract, dx_position):
      self.MinFract  = MinFract
      self.refValue  = assemble( project(Constant(1),U_AA)        *dx(dx_position) )
      self.smass     = assemble( TestFunction(U_AA)*Constant(1)   *dx(dx_position) )
      self.temp      = Function(U_AA)
   def function(self, m):
      print("Evaluting constraint residual")
      self.temp.vector()[:] = m
...
# problem_constraints = [VolConstraint(MinFract, sub_idx) for sub_idx in range(subdomain_qtd)]
...
class LowerBound(Expression):
   ...
   def eval_cell(self, values, x, ufc_cell):
      ...
      in_inlet    = 3*(x[0]-mesh_P0)**2.0 +(x[1]-mesh_inP)**2.0 <= (mesh_d/2.0 +mesh_tol)**2.0
      in_outlet   = 3*(x[0]-mesh_L )**2.0 +(x[1]-mesh_inP)**2.0 <= (mesh_d/2.0 +mesh_tol)**2.0
      if in_inlet or in_outlet:
...
adjProblem = MinimizationProblem(
   J_reduced,
   bounds         = [   interpolate(LowerBound(degree=1), U_AA),
                        interpolate(UpperBound(degree=1), U_AA)  ],
   constraints    = [],
   )
