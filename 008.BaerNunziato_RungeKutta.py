...
sigma1n = MU1*(grad(un1)+grad(un1).T) -pn1*Identity(len(un1))
sigma1m = MU1*(grad(um1)+grad(um1).T) -pm1*Identity(len(um1))
...
F1 = a1_df *b1                                     *dx \
   ...
   + inner(sigma1n*an1*0.5, grad(v1))              *dx \
   ...
   + inner(sigma2n*an2*0.5, grad(v2))              *dx \
